...
#define OWM_NUM_ALERTS                                                         \
  8 // OpenWeatherMaps does not specify a limit, but if you need more alerts you
    // are probably doomed.
#define OWM_NUM_AIR_POLLUTION                                                  \
  24 // Depending on AQI scale, hourly concentrations will need to be averaged
     // over a period of 1h to 24h

typedef struct owm_weather {
  int id;             // Weather condition id
  String main;        // Group of weather parameters (Rain, Snow, Extreme etc.)
  String description; // Weather condition within the group (full list of
...
typedef struct owm_current {
  ...
  float temp;       // Temperature. Units - default: kelvin, metric: Celsius,
                    // imperial: Fahrenheit.
  float feels_like; // Temperature. This temperature parameter accounts for the
                    // human perception of weather. Units – default: kelvin,
                    // metric: Celsius, imperial: Fahrenheit.
  int pressure;     // Atmospheric pressure on the sea level, hPa
  int humidity;     // Humidity, %
  float dew_point; // Atmospheric temperature (varying according to pressure and
  ...
  int visibility;  // Average visibility, metres. The maximum value of the
                   // visibility is 10km
...
typedef struct owm_hourly {
  ...
  float wind_gust;  // (where available) Wind gust. Units – default: metre/sec,
                    // metric: metre/sec, imperial: miles/hour.
  int wind_deg;     // Wind direction, degrees (meteorological)
  float pop; // Probability of precipitation. The values of the parameter vary
...
typedef struct owm_daily {
  ...
  float moon_phase; // Moon phase. 0 and 1 are 'new moon', 0.25 is 'first
                    // quarter moon', 0.5 is 'full moon' and 0.75 is 'last
                    // quarter moon'. The periods in between are called 'waxing
...
typedef struct owm_components {
  float co[OWM_NUM_AIR_POLLUTION];  // Сoncentration of CO (Carbon monoxide),
                                    // μg/m^3
  float no[OWM_NUM_AIR_POLLUTION];  // Сoncentration of NO (Nitrogen monoxide),
                                    // μg/m^3
  float no2[OWM_NUM_AIR_POLLUTION]; // Сoncentration of NO2 (Nitrogen dioxide),
                                    // μg/m^3
  float o3[OWM_NUM_AIR_POLLUTION];  // Сoncentration of O3 (Ozone), μg/m^3
  float so2[OWM_NUM_AIR_POLLUTION]; // Сoncentration of SO2 (Sulphur dioxide),
                                    // μg/m^3
  float pm2_5[OWM_NUM_AIR_POLLUTION]; // Сoncentration of PM2.5 (Fine particles
                                      // matter), μg/m^3
  float pm10[OWM_NUM_AIR_POLLUTION];  // Сoncentration of PM10 (Coarse
                                      // particulate matter), μg/m^3
...
typedef struct owm_resp_air_pollution {
  owm_coord_t coord;
  int main_aqi[OWM_NUM_AIR_POLLUTION]; // Air Quality Index. Possible values: 1,
                                       // 2, 3, 4, 5. Where 1 = Good, 2 = Fair,
